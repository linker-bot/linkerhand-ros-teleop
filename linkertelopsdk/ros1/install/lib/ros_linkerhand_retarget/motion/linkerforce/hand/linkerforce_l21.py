...
class RightHand:
    ...
    def joint_update(joint_arc) -> np.ndarray:
        ...
        qpos[0] = joint_arc[5] * -1
        qpos[1] = joint_arc[6]
        qpos[3] = 0
        if joint_arc[6] < joint_arc[8]:
            qpos[3] = abs(joint_arc[6] - joint_arc[8])
        # qpos[8] = joint_arc[9]
        # # if qpos[8] > 0 :
        # #     qpos[8] = 0
        # qpos[9] = joint_arc[10]
        # qpos[10] = joint_arc[11] * 2
        ...
        # # print(qpos[9])
        #
        # qpos[12] = joint_arc[13]
        # if qpos[12] < 0:
        #     qpos[12] = 0
        # qpos[13] = joint_arc[14]
        # qpos[14] = joint_arc[15] * 2
        ...
        # pass
        qpos[16] = joint_arc[1] * 0.5
        qpos[17] = joint_arc[0] * -1.5
        qpos[18] = joint_arc[2]
        qpos[19] = joint_arc[4] * -0.5
        qpos[20] = joint_arc[4] * -0.5
        return qpos
...
class LeftHand:
    ...
    def joint_update(joint_arc) -> np.ndarray:
        qpos = np.zeros(25)
        qpos[0] = joint_arc[5] * -1
        qpos[1] = joint_arc[7] * 3
        qpos[3] = (joint_arc[7] + joint_arc[8] - joint_arc[6]) * 1.5
        # if joint_arc[6] < joint_arc[8]:
        #     qpos[3] = abs(joint_arc[6] - joint_arc[8]) * 2

        qpos[4] = joint_arc[17] * -1
        qpos[5] = joint_arc[18] * 1.5
        qpos[7] = 0
        if joint_arc[20] < joint_arc[18]:
            qpos[7] = abs(joint_arc[20] - joint_arc[18]) * 2

        qpos[8] = joint_arc[9] * -1
        qpos[9] = joint_arc[10] * 1.5
        qpos[11] = 0
        if joint_arc[10] < joint_arc[12]:
            qpos[11] = qpos[9]

        qpos[12] = joint_arc[13] * -1
        qpos[13] = joint_arc[14] * 1.5
        qpos[15] = 0
        if joint_arc[16] < joint_arc[14]:
            qpos[15] = abs(joint_arc[16] - joint_arc[14]) * 2


        # pass
        qpos[16] = joint_arc[1] * 0.5
        qpos[17] = joint_arc[0] * -1
        qpos[18] = joint_arc[3] * 0.5
        qpos[19] = joint_arc[3] * 0.8
        qpos[20] = joint_arc[3] * 0.8
        return qpos
