...
class RightHand:
    ...
    def joint_update(self, joint_arc):
        ...
        self.O7_joint = [joint_arc[2],joint_arc[20],joint_arc[20],joint_arc[6],joint_arc[10],joint_arc[14],joint_arc[18]]
    ...
    def glove_torch(self,msg):
        value = self.O7_joint[3:]
        print("value",value)
        minvalue = min(value)
        min_index = value.index(minvalue)
        stone_pose = [-1.39,-1.39,-1.39,-1.39]
        weights_index_stone = [50, 50, 50, 50]
        weighted_diff_stone = [(j - p) * w for j, p, w in zip([self.O7_joint[3], self.O7_joint[4], self.O7_joint[5], self.O7_joint[6]], stone_pose, weights_index_stone)]
        threshold_stone = np.sqrt(np.sum(np.square(weighted_diff_stone)))
        print("threshold_stone",threshold_stone)
        if threshold_stone > 30:
         if min_index == 0:
           print("index---------")
           if self.O7_joint[3] < -1.3 * 0.5:
               print("index_probability_more---------")
               if self.O7_joint[0] <= 0.17:
                   print("index_probability_more_more---------")
                   index_torch_thumb_pose = [-0.12,0.15,0.15,-1.39]
                   weights_index = [10, 50, 50, 50]
                   weighted_diff = [(j - p) * w for j, p, w in zip([self.O7_joint[0], self.O7_joint[1], self.O7_joint[2], self.O7_joint[3]], index_torch_thumb_pose, weights_index)]
         ...
         if min_index == 1:
           print("middle--------")
           if self.O7_joint[4] < -1.3 * 0.5:
               print("middle_probability_more---------")
               if self.O7_joint[0] < 0:
                   print("middle_probability_more_more---------")
                   middle_torch_thumb_pose = [-0.44,0.31,0.31,-1.39]
                   weights_middle = [10, 50, 50, 50]
                   weighted_diff = [(j - p) * w for j, p, w in zip([self.O7_joint[0], self.O7_joint[1], self.O7_joint[2], self.O7_joint[4]], middle_torch_thumb_pose, weights_middle)]
                   threshold = np.sqrt(np.sum(np.square(weighted_diff)))
                   print("threshold",threshold)
                   if threshold < 30:
                      print("middle_torch")
         ...
         if min_index == 2:
           ...
           if self.O7_joint[5] < -1.3 * 0.5:
               ...
               if self.O7_joint[0] < 0:
                   ...
                   weighted_diff = [(j - p) * w for j, p, w in zip([self.O7_joint[0], self.O7_joint[1], self.O7_joint[2], self.O7_joint[5]], ring_torch_thumb_pose, weights_ring)]
                   threshold = np.sqrt(np.sum(np.square(weighted_diff)))
                   print("threshold",threshold)
                   if threshold < 30:
                      print("ring_torch")
         ...
         if min_index == 3:
           ...
           if self.O7_joint[6] < -1 * 0.5:
               ...
               if self.O7_joint[0] < 0:
                   ...
                   weighted_diff = [(j - p) * w for j, p, w in zip([self.O7_joint[0], self.O7_joint[1], self.O7_joint[2], self.O7_joint[6]], little_torch_thumb_pose, weights_little)]
                   threshold = np.sqrt(np.sum(np.square(weighted_diff)))
                   print("threshold",threshold)
                   if threshold < 40 :
                      print("little_torch")
