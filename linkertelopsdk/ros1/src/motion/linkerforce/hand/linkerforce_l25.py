...
class LeftHand:
    ...
    def joint_update(self, joint_arc):
        ...
        qpos[4] = 0
        qpos[5] = (joint_arc[18] - 0.7) * 4
        if joint_arc[18] < 0.2 and joint_arc[19] < 0.2 and joint_arc[20] < 0.2:
            # print("手指伸直状态", joint_arc[18], joint_arc[19], joint_arc[20])
            qpos[6] = 0
            qpos[7] = 0
        elif joint_arc[18] > joint_arc[19] > joint_arc[20]:
            # print("2，3关节伸直状态", joint_arc[18], joint_arc[19], joint_arc[20])
            qpos[6] = joint_arc[20] / 2
            qpos[7] = joint_arc[20] / 2
        else:
            if joint_arc[11] < 0.6 and joint_arc[12] < 0.6:
                # print("2，3关节伸直状态", joint_arc[18], joint_arc[19], joint_arc[20])
                qpos[10] = joint_arc[20] / 2
                qpos[11] = joint_arc[20] / 2
            else:
                # print("2，3关节弯曲状态", joint_arc[18], joint_arc[19], joint_arc[20])
                qpos[6] = joint_arc[20]
                qpos[7] = joint_arc[20]
        ...
        if joint_arc[10] < 0.2 and joint_arc[11] < 0.2 and joint_arc[12] < 0.2:
            ...
            # print("2，3关节伸直状态", joint_arc[10],joint_arc[11],joint_arc[12])
            qpos[9] = (joint_arc[10] - 0.7) * 4
            qpos[10] = joint_arc[12] / 2
            qpos[11] = joint_arc[12] / 2
        else:
            if joint_arc[11] < 0.6 and joint_arc[12] < 0.5:
                # print("2，3关节伸直状态", joint_arc[10], joint_arc[11], joint_arc[12])
                qpos[9] = (joint_arc[10] - 0.7) * 4
                qpos[10] = joint_arc[12] / 2
                qpos[11] = joint_arc[12] / 2
            ...
            #     qpos[11] = joint_arc[10] / 2
                ...
                # print("2，3关节弯曲状态", joint_arc[10], joint_arc[11], joint_arc[12], (joint_arc[10] - 0.7) * 4)
                qpos[9] = (joint_arc[10] - 0.7) * 4
                qpos[10] = joint_arc[12]
                qpos[11] = joint_arc[12]
        ...
        if joint_arc[14] < 0.2 and joint_arc[15] < 0.2 and joint_arc[16] < 0.2:
            ...
            # print("2，3关节伸直状态1", joint_arc[14],joint_arc[15],joint_arc[16])
            qpos[14] = joint_arc[16] / 2
            qpos[15] = joint_arc[16] / 2
        else:
            if joint_arc[15] < 0.6 and joint_arc[16] < 0.6:
                # print("2，3关节伸直状态2", joint_arc[14], joint_arc[15], joint_arc[16])
                qpos[14] = joint_arc[16] / 2
                qpos[15] = joint_arc[16] / 2
            else:
        ...
        qpos[19] = 0
        qpos[20] = 0
        if joint_arc[2] < 0.2 and joint_arc[3] < 0.2 and joint_arc[4] < 0.2:
            ...
            qpos[19] = joint_arc[2] / 2
            qpos[20] = joint_arc[4]
        else:
            if joint_arc[2] > joint_arc[4]:
                # print("1关节弯曲状态", joint_arc[18], joint_arc[19], joint_arc[20])
                qpos[18] = joint_arc[2] / 4
                qpos[19] = joint_arc[2]
                qpos[20] = joint_arc[4]
            else:
